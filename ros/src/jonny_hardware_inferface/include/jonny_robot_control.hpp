...
class JonnyRobotControl {
  ...
  public: 
    ...
    struct RobotConstants {
      static constexpr double AXIS_ZERO_POSITION[6] = {-180, 91, 118, 0, 90, 78}; // Amount to move from endstop to zero
      static constexpr double AXIS_RATIO[6] = {14, 150, 150, 45, 36, 36}; // axis ratio
      static constexpr double MOTOR_SET_INVERTED[6] = {1, 1,  1, -1, -1, 1}; // invert position when sending commands
      static constexpr double MOTOR_GET_INVERTED[6] = {1, 1, -1, -1, -1, 1}; // invert position when reading from motor
    };
