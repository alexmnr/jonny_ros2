...
bool JennyMotorControl::setRelativeXYZAJointPosition(uint8_t id, double position, double speed, double acceleration) {
  rclcpp::Logger logger = rclcpp::get_logger("JennyJointInterface");
  if (id > 3) {
...
bool JennyMotorControl::setAbsoluteXYZAJointPosition(uint8_t id, double position, double speed, double acceleration) {
  rclcpp::Logger logger = rclcpp::get_logger("JennyJointInterface");
  if (id > 3) {
    RCLCPP_ERROR(logger, "Wrong Joint for XYZA Control!");
    return false;
...
bool JennyMotorControl::setRelativeBCJointPosition(double position[2], double speed, double acceleration) {
...
bool JennyMotorControl::setAbsoluteBCJointPosition(double position[2], double speed, double acceleration) {
  double motor_5_position = (position[0] - position[1]) * RobotConstants::AXIS_SET_INVERTED[4];
  double motor_6_position = (position[0] + position[1]) * RobotConstants::AXIS_SET_INVERTED[5];
  double motor_5_speed = speed;
  double motor_6_speed = speed;
